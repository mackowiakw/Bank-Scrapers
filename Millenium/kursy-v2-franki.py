...
fields = ('foreignExchangeBuy', 'foreignExchangeSale')
...
for date_requested in tqdm(dates):
    ...
    date_requested_YYYYMMDD = datetime.strptime(
        date_requested, '%d-%m-%Y').strftime('%Y-%m-%d')

    url = f'https://www.bankmillennium.pl/portal-apps/getFxRates?date={date_requested}&language=pl'

    response = requests.get(url, headers=headers)
    response_json = sorted(json.loads(response.content), key=lambda x: x['timeStamp'])
    ...
    for record in response_json:
        ...
        if record['chh']:
            result += str(record['chh']['foreignExchangeBuy']) + '\t'
            result += str(record['chh']['foreignExchangeSale'])
        else:
            result += '---'
...
with open("millenium-chh.txt", "w") as result_file:
