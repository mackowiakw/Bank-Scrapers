...
# 1. pobrać wszystkie daty dla BNP (robić check czy na pewno jest BNP)
# 1a. mieć listę dat gdzie występuje też BGZ (robić check czy nie ma jeszcze innego banku)
# Odczytać tę listę dat
# 2. pobrać daty z BGZ - zmienić pole BANK_CODE na 'bgz'

THREADS_NUMBER = 5
BANK_CODE = 'bnp'  # 'bnp' or 'bgz'

FILENAME = f'bnpparibas-kursy-walut-{BANK_CODE.upper()}'

HEADER = f'''BNP Paribas: Kursy walut
TABELA KURSÓW WALUT BNP PARIBAS BANK POLSKA S.A. (CHF)
...
FIELDS = ('Data', 'Nr tabeli Godzina', 'Bank', 'Kod tabeli',
...
headers = {
    ...
    'referer': 'https://www.bnpparibas.pl/kursy-walut?ratesTable=27672',
...
def getRates(date):
    ...
    for (code, num_hour) in options:
        ...
        # if table_date != date_check:
        #     return "Got another date"
        # if table_code != ratesTable:
        #     return "Got another ratesTable"
        # if table_num_hour != num_hour_check:
        #     return "Got another num_hour"
        # if table_bank != bankCode:
        #     return "Got another bank"

        # <table class="rates_table default-table" aria-describedby="rates-3485195">
        #   <tbody>
        #     <tr class="bgz_bnp_exchange_rates__tableBodyRow" data-index="14">
        ...
        #       <td>4,7979</td>
        #       <td>0,2340</td>
        #       <td>4,6809</td>     # Pole dodane później
        #     </tr>
        #   </tbody>
        # </table>

        for entry in soup \
            .find("table", {"class": "rates_table", "aria-describedby": "rates-3485195"}) \
                .find("tbody") \
            ...
            if '1 CHF' == entry.find("td", {"class": "bgz_bnp_exchange_rates__currency"}).text.strip():
                ...
                mean = tds[5].text.strip() if len(tds) == 6 else ''

                row = (table_date, table_num_hour, table_bank,
                       table_code, buy, sell, spread, mean)
                result += '\t'.join([str(x) for x in row]) + '\n'
                break

        else:
            row = (table_date, table_num_hour, table_bank, table_code)
